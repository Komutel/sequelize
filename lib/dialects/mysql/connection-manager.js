...
class MySqlConnectionManager extends import_connection_manager.AbstractConnectionManager {
  ...
  async connect(config) {
    (0, import_assert.default)(typeof config.port === "number", "port has not been normalized");
    ...
    try {
      ...
      connection.on("error", (error) => {
        if (!(0, import_utils.isNodeError)(error)) {
          return;
        }
        switch (error.code) {
          case "ESOCKET":
      ...
      if (!this.sequelize.config.keepDefaultTimezone && this.sequelize.options.timezone) {
        ...
        await (0, import_util.promisify)((cb) => connection.query(`SET time_zone = '${tzOffset}'`, cb))();
      }
      return connection;
    } catch (error) {
      ...
      const code = (0, import_utils.isNodeError)(error) ? error.code : null;
      switch (code) {
        case "ECONNREFUSED":
  ...
  async disconnect(connection) {
    ...
    await (0, import_util.promisify)((callback) => connection.end(callback))();
  }
  validate(connection) {
