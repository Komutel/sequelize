      ...
      if (_.startsWith(this.sql, "BEGIN TRANSACTION")) {
        ...
        connection.prepare(newSql, (err, stmt) => {
          ...
          stmt.execute(params, (err2, result, outparams) => {
            ...
            if (benchmark) {
              this.sequelize.log(`${queryLabel}Executed (${this.connection.uuid || "default"}): ${newSql} ${parameters ? import_node_util.default.inspect(parameters, { compact: true, breakLength: Infinity }) : ""}`, Date.now() - queryBegin, this.options);
            }
