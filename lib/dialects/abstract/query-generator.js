...
class AbstractQueryGenerator {
      ...
      if (Object.prototype.hasOwnProperty.call(valueHash, key)) {
        ...
        if (modelAttributeMap && modelAttributeMap[key] && modelAttributeMap[key].autoIncrement === true && value == null) {
          ...
          if (value instanceof Utils.SequelizeMethod || options.bindParam === false) {
            values.push(this.escape(value, modelAttributeMap && modelAttributeMap[key] || void 0, { context: "INSERT", replacements: options.replacements }));
          } else {
  ...
  updateQuery(tableName, attrValueHash, where, options, columnDefinitions) {
    ...
    if (this._dialect.supports["LIMIT ON UPDATE"] && options.limit && this.dialect !== "mssql" && this.dialect !== "db2") {
      suffix = ` LIMIT ${this.escape(options.limit, void 0, options)} `;
    }
    if (this._dialect.supports.returnValues && options.returning) {
  ...
  arithmeticQuery(operator, tableName, where, incrementAmountsByField, extraAttributesToBeUpdated, options) {
    ...
    for (const field in incrementAmountsByField) {
      ...
      const escapedAmount = this.escape(incrementAmount, void 0, replacementOptions);
    ...
    for (const field in extraAttributesToBeUpdated) {
      ...
      const quotedField = this.quoteIdentifier(field);
      const escapedValue = this.escape(newValue, void 0, replacementOptions);
  ...
  addIndexQuery(tableName, attributes, options, rawTablename) {
    ...
    const escapedIndexName = tableName.schema && this.dialect === "db2" ? this.quoteTable({
    ...
    ind = ind.concat(
      ...
      options.type,
      "INDEX",
      !this._dialect.supports.indexViaAlter ? concurrently : void 0,
      escapedIndexName,
      this._dialect.supports.index.using === 1 && options.using ? `USING ${options.using}` : "",
      !this._dialect.supports.indexViaAlter ? `ON ${escapedTableName}` : void 0,
      this._dialect.supports.index.using === 2 && options.using ? `USING ${options.using}` : "",
      `(${fieldsSql.join(", ")})`,
      this._dialect.supports.index.parser && options.parser ? `WITH PARSER ${options.parser}` : void 0,
      this._dialect.supports.index.where && options.where ? options.where : void 0
    );
  ...
  getConstraintSnippet(tableName, options) {
    ...
    switch (options.type.toUpperCase()) {
      ...
      case "DEFAULT":
        ...
        constraintSnippet = `CONSTRAINT ${constraintName} DEFAULT (${this.escape(options.defaultValue, void 0, options)}) FOR ${fieldsSql[0]}`;
        break;
      case "PRIMARY KEY":
  ...
  quote(collection, parent, connector = ".", options) {
    ...
    if (collection instanceof Utils.SequelizeMethod) {
      return this.handleSequelizeMethod(collection, void 0, void 0, options);
    }
    ...
    if (subQuery) {
      ...
      if (options.groupedLimit) {
        ...
        if (options.groupedLimit.on instanceof BelongsToMany) {
          ...
          const groupedLimitOptions = _validateIncludedElements({
            include: [{
              ...
              where: {
                [Op.placeholder]: true,
                ...(_a = options.groupedLimit.through) == null ? void 0 : _a.where
              }
            }],
            model
          });
          ...
          if (Array.isArray(options.order)) {
            options.order.forEach((order, i) => {
              ...
              alias = this.sequelize.literal(this.quote(alias, void 0, void 0, options));
        ...
        mainQueryItems.push(this.selectFromTableFragment(options, mainTable.model, attributes.main, `(${options.groupedLimit.values.map((value) => {
          ...
          return Utils.spliceStr(baseQuery, splicePos, placeHolder.length, this.getWhereConditions(groupWhere, groupedTableName, void 0, options));
        }).join(
  ...
  escapeAttributes(attributes, options, mainTableAs) {
    return attributes && attributes.map((attr) => {
      ...
      if (attr instanceof Utils.SequelizeMethod) {
        return this.handleSequelizeMethod(attr, void 0, void 0, options);
      }
      if (Array.isArray(attr)) {
        ...
        if (attr[0] instanceof Utils.SequelizeMethod) {
          attr[0] = this.handleSequelizeMethod(attr[0], void 0, void 0, options);
          addTable = false;
        ...
        attr = !attr.includes(Utils.TICK_CHAR) && !attr.includes('"') ? this.quoteAttribute(attr, options.model) : this.escape(attr, void 0, options);
      }
    ...
    if (topLevelInfo.options.includeIgnoreAttributes !== false) {
      ...
      const includeAttributes = include.attributes.map((attr) => {
        ...
        if (Array.isArray(attr) && attr.length === 2) {
          ...
          attr = attr.map((attrPart) => attrPart instanceof Utils.SequelizeMethod ? this.handleSequelizeMethod(attrPart, void 0, void 0, options) : attrPart);
          attrAs = attr[1];
          attr = attr[0];
        }
    ...
    if (Array.isArray(options.order)) {
      for (let order of options.order) {
        ...
        if (subQuery && Array.isArray(order) && order[0] && !(order[0] instanceof Association) && !(0, import_model_utils.isModelStatic)(order[0]) && !(0, import_model_utils.isModelStatic)(order[0].model) && !(typeof order[0] === "string" && model && model.associations !== void 0 && model.associations[order[0]])) {
          const field = ((_a = model.rawAttributes[order[0]]) == null ? void 0 : _a.field) || order[0];
  ...
  addLimitAndOffset(options, model) {
    ...
    if (options.limit != null) {
      fragment += ` LIMIT ${this.escape(options.limit, void 0, options)}`;
    } else if (options.offset) {
      fragment += ` LIMIT 18446744073709551615`;
    }
    if (options.offset) {
      fragment += ` OFFSET ${this.escape(options.offset, void 0, options)}`;
    }
    return fragment;
  }
  handleSequelizeMethod(smth, tableName, factory, options, prepend) {
    ...
    if (smth instanceof Utils.Where) {
      ...
      if ([this.OperatorMap[Op.between], this.OperatorMap[Op.notBetween]].includes(smth.comparator)) {
        value = `${this.escape(value[0], void 0, options)} AND ${this.escape(value[1], void 0, options)}`;
      } else if (typeof value === "boolean") {
        value = this.booleanValue(value);
      } else {
        value = this.escape(value, void 0, options);
      }
      if (value === "NULL") {
        if (smth.comparator === "=") {
          smth.comparator = "IS";
    ...
    if (smth instanceof Utils.Literal) {
      if (options == null ? void 0 : options.replacements) {
        return (0, import_sql.injectReplacements)(smth.val, this._dialect, options.replacements, {
...
\u279C literal(${JSON.stringify(smth.val)})`);
    ...
    if (smth instanceof Utils.Cast) {
      if (smth.val instanceof Utils.SequelizeMethod) {
        ...
        result = this.escape(smth.val, void 0, options);
      }
    ...
    if (smth instanceof Utils.Fn) {
      return `${smth.fn}(${smth.args.map((arg) => {
        ...
        return this.escape(arg, void 0, options);
      }).join(", ")})`;
    }
    if (smth instanceof Utils.Col) {
      ...
      return this.quote(smth.col, factory, void 0, options);
    }
  ...
  whereItemQuery(key, value, options = {}) {
    ...
    if (typeof key === "string" && key.includes(".") && options.model) {
      ...
      if (options.model.rawAttributes[keyParts[0]] && options.model.rawAttributes[keyParts[0]].type instanceof DataTypes.JSON) {
        ...
        return this.whereItemQuery(field2.field || keyParts[0], tmp, { field: field2, ...options });
      }
    }
    ...
    if (value === null) {
      const opValue2 = options.bindParam ? "NULL" : this.escape(value, field, options);
      return this._joinKeyValue(key, opValue2, this.OperatorMap[Op.is], options.prefix);
    }
    if (!value) {
      const opValue2 = options.bindParam ? this.format(value, field, options, options.bindParam) : this.escape(value, field, options);
  ...
  _whereParseSingleValueObject(key, field, prop, value, options) {
    ...
    switch (prop) {
      ...
      case Op.all:
        ...
        if (value[Op.values]) {
          return this._joinKeyValue(key, `(VALUES ${value[Op.values].map((item) => `(${this.escape(item, void 0, options)})`).join(", ")})`, comparator, options.prefix);
        }
      ...
      case Op.notSubstring: {
        ...
        return this._joinKeyValue(key, this.escape(pattern, void 0, options), comparator, options.prefix);
      }
    }
    const escapeOptions = {
  ...
  getWhereConditions(smth, tableName, factory, options, prepend) {
    ...
    if (Buffer.isBuffer(smth)) {
      return this.escape(smth, void 0, options);
    }
