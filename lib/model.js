...
Specify a different name for either index to resolve this issue.`);
  ...
  static init(attributes, options = {}) {
    ...
    this._manualIndexes = this.options.indexes.map((index) => this._nameIndex(this._conformIndex(index)));
    this.primaryKeys = /* @__PURE__ */ Object.create(null);
    this._readOnlyAttributes = /* @__PURE__ */ new Set();
    this._timestampAttributes = /* @__PURE__ */ Object.create(null);
  ...
  static refreshAttributes() {
    ...
    _.each(this.rawAttributes, (definition, name) => {
      ...
      if (Object.prototype.hasOwnProperty.call(definition, "unique") && definition.unique) {
        ...
        const index = definition.unique.name && this.uniqueKeys[definition.unique.name] ? this.uniqueKeys[definition.unique.name] : { fields: [] };
        ...
        (_a = definition.unique).name ?? (_a.name = index.name);
      ...
      if (options.alter === true || typeof options.alter === "object" && options.alter.drop !== false) {
        for (const columnName in columns) {
          ...
          if (currentAttribute.references) {
            ...
            const foreignReferenceTableName = typeof references.model === "object" ? references.model.tableName : references.model;
...
`.trim());
  ...
  async save(options) {
    ...
    if (this.sequelize.options.dialect === "db2" && this.isNewRecord) {
      this.uniqno = this.sequelize.dialect.queryGenerator.addUniqueFields(
        this.dataValues,
        this.constructor.rawAttributes,
        this.uniqno
      );
    }
    if (options.validate) {
    ...
    if (wasNewRecord && this._options.include && this._options.include.length > 0) {
      await Promise.all(
        this._options.include.filter((include) => !(include.association instanceof BelongsTo || include.parent && include.parent.association instanceof BelongsToMany)).map(async (include) => {
